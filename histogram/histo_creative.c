...
long* histogram(char* fn_input) {
  ...
  // Initialize a private histogram for each thread
  int** private_histos = (int**)malloc(num_threads * sizeof(int*));
  for (i = 0; i < num_threads; i++) {
    private_histos[i] = (int*)calloc(256 + 1, sizeof(int));
  }

  t_start = omp_get_wtime();
  #pragma omp parallel num_threads(num_threads) private(i, j, m)
  {
  int tid = omp_get_thread_num();

  int chunk_size = image->row / num_threads;
  int start_row = tid * chunk_size;
  int end_row = (tid == num_threads - 1) ? image->row : start_row + chunk_size;

  /* obtain histogram from image, repeated 100 times */
  for (m=0; m<100; m++) {
    for (i=start_row; i<end_row; i++) {
      for (j=0; j<image->col; j++) {
	private_histos[tid][image->content[i][j]]++;
      }
    }
  }
